...
def save_images(webpage, visuals, image_path, aspect_ratio=1.0, width=256):
    ...
    参数：webpage（HTML类）-存储这些图像的HTML网页类（有关更多详细信息，请参见html.py）视觉效果（OrderedDict）-存储（名称，
    图像（张量或numpy）的对）的有序词典image_path（str）-
    用于创建图像路径的字符串Aspect_ratio（float）-已保存图像的宽高比width（int）-
    图像将被调整为width x width此功能将保存存储在'visuals中的图像”到“网页”指定的HTML文件中
