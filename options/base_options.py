...
class BaseOptions():
    ...
    它还收集数据集类和模型类中<modify\u commandline\u options>函数中定义的其他选项。
    """
    ...
    def gather_options(self):
        """使用基本选项初始化解析器（仅一次）。添加其他特定于模型和特定于数据集的选项。
        这些选项在model和dataset类中的<modify\u commandline\u options>函数中定义。
        """
    ...
    def print_options(self, opt):
        ...
        它将打印当前选项和默认值（如果不同）。它将选项保存到文本文件[checkpoints\u dir]opt.txt中
        """
        message = ''
        message += '----------------- Options ---------------\n'
